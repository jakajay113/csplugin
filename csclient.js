            ...
            if (chat.message !== "" || chat.file_path !== "") {
                ...
                if (chat.file_path !== "") {
                    ...
                    if (chat.file_type === "image/jpeg" || chat.file_type === "image/png") {
                        fileElement = `<img src="${'https://eservices.bagocity.gov.ph/spidc_web_api_test/CSPluginServer'+chat.file_path}" alt="Attachment" onclick="viewFullScreen('${'https://eservices.bagocity.gov.ph/spidc_web_api_test/CSPluginServer'+chat.file_path}')" style="cursor: pointer;width: 100%;height: 100px;">`;
                        ...
                        fileElement = `<video controls><source src="${chat.file_path}" type="video/mp4"></video>`;
                            ...
                            if(messageClass === "message-box"){
                                fileElement = `<a href="${'https://eservices.bagocity.gov.ph/spidc_web_api_test/CSPluginServer'+chat.file_path}" download="${chat.file_path.split('/').pop()}" style="color: white;">${chat.file_path.split('/').pop()}</a>`;
                            }else{
                                fileElement = `<a href="${'https://eservices.bagocity.gov.ph/spidc_web_api_test/CSPluginServer'+chat.file_path}" download="${chat.file_path.split('/').pop()}" style="color: #716060;">${chat.file_path.split('/').pop()}</a>`;
                            }
                ...
                chatMessage += '</div>\n'  // Close message-box or bot-box
                               +''+chatMessageToolTip+'\n'
                            +'<span class="'+cstimepostion+'">' + timeAgo(chat.sent_at) + '</span>\n'
        ...
        const dsfsdfsdf = {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    const imageRegex = new RegExp(`(<div class="message-box">)(\\s*(<div style="margin-top: 0px;">\\s*<img[^>]*>\\s*</div>))(\\s*</div>\\s*<span class="newtooltip-text csthaction" data-id="${targetId}".*?>)`, 'i');
                    // Create regex to find the message box containing an <a>
                    const linkRegex = new RegExp(`(<div class="message-box">)(\\s*(<div style="margin-top: 0px;">\\s*<a[^>]*>.*?</a>\\s*</div>))(\\s*</div>\\s*<span class="newtooltip-text csthaction" data-id="${targetId}".*?>)`, 'i');
